...
{items.map((item) => {
  return (
    <div className="item d-flex align-center mb-30">
      ...
      <img onClick={() =>onRemove(item.id)} className=" remove" src="remove.svg" alt="remove" />
    </div>
  );
})}
</div>
...
<img className="mb-20" width={120} height={120}  src="empty-cart.jpg" />
  <h2>Cart is empty</h2>
  <p>Add at least one sneaker, for making order.</p>
  <button onClick={onClickClose} className="greenButton">Go back</button>
</div>
